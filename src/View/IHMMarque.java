 ...
 * @author Moustaid
 */
public class IHMMarque extends javax.swing.JFrame {
private DateGetter dateGetter=new DateGetter();
private MarqueTableModel marqueTableModel=new MarqueTableModel();
private MarqueDAO mDAO=new MarqueDAO(SingletonConnexion.getInstance());
private JTable tableMarques;
private JScrollPane jsp=new JScrollPane();
private List<Marque> lm=new ArrayList();
    /**
    ...
    private void initComponents() {
        ...
        setTitle("Gestion Véhicules OCP - Marques");
        setResizable(false);
        ...
        labRechercherPar.setText("Rechercher une marque : ");
...
labName.setText(IHMLogin.user.getNomUser().toUpperCase()+" "+IHMLogin.user.getPrenomUser());
        tableMarques=new JTable(marqueTableModel);
     tableMarques.setVisible(true);
     tableMarques.setAutoCreateRowSorter(true);
      tableMarques.setShowGrid(true);
     tableMarques.setGridColor(Color.GRAY);
          jsp.setBorder(null);
     jsp.setViewportView(tableMarques);
     jsp.getViewport().setOpaque(false);
jsp.setOpaque(false);
     panelTable.add(jsp);
              setTableMarqueEvent();
        lm=mDAO.readAll(); 
marqueTableModel=(MarqueTableModel) tableMarques.getModel();
marqueTableModel.loadData(lm);// TODO add your handling code here:
    }//GEN-LAST:event_formWindowOpened

    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
String j=JOptionPane.showInputDialog(tableMarques,"Nom Marque : ","Ajout d'une Marque",1);
if(j!=null && mDAO.findByPartOf(j).size()==0){
Marque m=new Marque();
m.setNomMarque(j);
mDAO.create(m);
JOptionPane.showMessageDialog(tableMarques,"Marque Ajoutée !" );
lm=mDAO.readAll(); 
marqueTableModel=(MarqueTableModel) tableMarques.getModel();
marqueTableModel.loadData(lm);
}
else
    JOptionPane.showMessageDialog(this, "Nom marque exist déja", "Attention", 0);

    }//GEN-LAST:event_btnAddActionPerformed

    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
int s=tableMarques.getSelectedRow();
if(s!=-1)
{
    int i=JOptionPane.showConfirmDialog
        (this, "Attention tous les objets liés à cette Marque seront supprimés ! \n Voulez-vous vraiment supprimer la Marque : "+tableMarques.getValueAt(s, 0).toString()+"?");
    if(i==0){
mDAO.delete(Integer.parseInt(String.valueOf(tableMarques.getValueAt(s, 0))));
        lm=mDAO.readAll();
        marqueTableModel=(MarqueTableModel) tableMarques.getModel();
marqueTableModel.loadData(lm);
...
{
 JOptionPane.showMessageDialog(this,"Veuillez sélectionner la Marque à supprimer dans le tableau" );
}        // TODO add your handling code here:
...
int s=tableMarques.getSelectedRow();
if(s!=-1){
 String j=JOptionPane.showInputDialog(tableMarques,
         "Nom Marque : ("+String.valueOf(tableMarques.getValueAt(s, 1))+")","Modification d'une Marque",1);
if(j!=null && mDAO.findByPartOf(j).size()==0){
Marque m=new Marque();
m.setIdMarque(Integer.parseInt(String.valueOf(tableMarques.getValueAt(s, 0))));
m.setNomMarque(j);
mDAO.update(m);
JOptionPane.showMessageDialog(tableMarques,"Marque Modifiée !" );
lm=mDAO.readAll(); 
marqueTableModel=(MarqueTableModel) tableMarques.getModel();
marqueTableModel.loadData(lm);
}
...
{
 JOptionPane.showMessageDialog(this,"Veuillez sélectionner la Marque à modifier dans le tableau" );
}
...
txtRecherche.setText("Entrez valeur ...");        // TODO add your handling code here:
    ...
    private void txtRechercheKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtRechercheKeyTyped
     ...
     {marqueTableModel.loadData(mDAO.findByPartOf(txtRecherche.getText()));  
     }
    else
     {
    marqueTableModel.loadData(mDAO.findByPartOf(txtRecherche.getText()+evt.getKeyChar()));  
...
try {
                        ...
                        Logger.getLogger(IHMMarque.class.getName()).log(Level.SEVERE, null, ex);
...
public void setTableMarqueEvent() {
        tableMarques.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                if(tableMarques.getSelectedRow()!=-1)
                {
                    Marque m=new Marque();
                    m=mDAO.read(Integer.parseInt(String.valueOf(tableMarques.getValueAt(tableMarques.getSelectedRow(), 0))));
                    JOptionPane.showMessageDialog(tableMarques, "<html></body><h1>Informations Marque</h1><table>"
                            + "<tr><td>Numero :</td><td><b>"+m.getIdMarque()+"</b></td></tr>"
                            + "<tr><td>Nom Marque :</td><td><b>"+m.getNomMarque()+"</b></td></tr>"
                            +"</table><br/></body></html>", "Informations Marque", 1);
