...
import Utiles.DivisionTableModel;
import Utiles.MarqueTableModel;
 ...
 * @author Moustaid
 */
public class IHMDivision extends javax.swing.JFrame {
private DateGetter dateGetter=new DateGetter();
private DivisionTableModel divisionTableModel=new DivisionTableModel();
private DivisionDAO dDAO=new DivisionDAO(SingletonConnexion.getInstance());
private JTable tableDivision;
private JScrollPane jsp=new JScrollPane();
private List<Division> ld=new ArrayList();
    /**
    ...
    private void initComponents() {
        ...
        setTitle("Gestion Véhicules OCP - Divisions");
        setResizable(false);
        ...
        labRechercherPar.setText("Rechercher une division : ");
...
labName.setText(IHMLogin.user.getNomUser().toUpperCase()+" "+IHMLogin.user.getPrenomUser());
        ...
        tableDivision=new JTable(divisionTableModel);
     tableDivision.setVisible(true);
     tableDivision.setAutoCreateRowSorter(true);
      tableDivision.setShowGrid(true);
     tableDivision.setGridColor(Color.GRAY);
          jsp.setBorder(null);
     jsp.setViewportView(tableDivision);
     jsp.getViewport().setOpaque(false);
jsp.setOpaque(false);
     panelTable.add(jsp);
              setTableDivisionEvent();
        ld=dDAO.readAll(); 
divisionTableModel=(DivisionTableModel) tableDivision.getModel();
divisionTableModel.loadData(ld);// TODO add your handling code here:
    }//GEN-LAST:event_formWindowOpened

    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
String j=JOptionPane.showInputDialog(tableDivision,"Nom Division : ","Ajout d'une Division",1);
if(j!=null && dDAO.findByPartOf(j).size()==0){
Division d=new Division();
d.setNomDivision(j);
dDAO.create(d);
JOptionPane.showMessageDialog(tableDivision,"Division Ajoutée !" );
ld=dDAO.readAll(); 
divisionTableModel=(DivisionTableModel) tableDivision.getModel();
divisionTableModel.loadData(ld);
}
else
    JOptionPane.showMessageDialog(this,"Nom de division existe déja !" );
    }//GEN-LAST:event_btnAddActionPerformed

    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
int s=tableDivision.getSelectedRow();
if(s!=-1)
{
    int i=JOptionPane.showConfirmDialog
        (this, "Attention tous les objets liés à cette Division seront supprimés ! \n Voulez-vous vraiment supprimer la Marque : "+tableDivision.getValueAt(s, 0).toString()+"?");
    if(i==0){
dDAO.delete(Integer.parseInt(String.valueOf(tableDivision.getValueAt(s, 0))));
        ld=dDAO.readAll();
        divisionTableModel=(DivisionTableModel) tableDivision.getModel();
divisionTableModel.loadData(ld);
...
{
 JOptionPane.showMessageDialog(this,"Veuillez sélectionner la Division à supprimer dans le tableau" );
}        // TODO add your handling code here:
...
int s=tableDivision.getSelectedRow();
if(s!=-1){
 String j=JOptionPane.showInputDialog(tableDivision,
         "Nom Division : ("+String.valueOf(tableDivision.getValueAt(s, 1))+")","Modification d'une Division",1);
if(j!=null && dDAO.findByPartOf(j).size()==0){
Division d=new Division();
d.setIdDivision(Integer.parseInt(String.valueOf(tableDivision.getValueAt(s, 0))));
d.setNomDivision(j);
dDAO.update(d);
JOptionPane.showMessageDialog(tableDivision,"Division Modifiée !" );
ld=dDAO.readAll(); 
divisionTableModel=(DivisionTableModel) tableDivision.getModel();
divisionTableModel.loadData(ld);
}
...
{
 JOptionPane.showMessageDialog(this,"Veuillez sélectionner la Division à modifier dans le tableau" );
}
...
txtRecherche.setText("Entrez valeur ...");        // TODO add your handling code here:
    ...
    private void txtRechercheKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtRechercheKeyTyped
     ...
     {divisionTableModel.loadData(dDAO.findByPartOf(txtRecherche.getText()));  
     }
    else
     {
    divisionTableModel.loadData(dDAO.findByPartOf(txtRecherche.getText()+evt.getKeyChar()));  
...
try {
                        ...
                        Logger.getLogger(IHMDivision.class.getName()).log(Level.SEVERE, null, ex);
...
public void setTableDivisionEvent() {
        tableDivision.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                if(tableDivision.getSelectedRow()!=-1)
                {
                    Division d=new Division();
                    d=dDAO.read(Integer.parseInt(String.valueOf(tableDivision.getValueAt(tableDivision.getSelectedRow(), 0))));
                    JOptionPane.showMessageDialog(tableDivision, "<html></body><h1>Informations Division</h1><table>"
                            + "<tr><td>Numero :</td><td><b>"+d.getIdDivision()+"</b></td></tr>"
                            + "<tr><td>Nom Division :</td><td><b>"+d.getNomDivision()+"</b></td></tr>"
                            +"</table><br/></body></html>", "Informations Division", 1);
